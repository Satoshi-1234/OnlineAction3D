...
public abstract class StringSelectDrawerBase<TAttr> : PropertyDrawer where TAttr : PropertyAttribute
{
	public sealed override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
	{
		...
		{
			EditorGUI.LabelField(position, label.text, "String only");
		...
		{
			EditorGUI.LabelField(position, label.text, string.IsNullOrEmpty(emptyMessage) ? "No options" : emptyMessage);
			EditorGUI.EndProperty();
			return;
		}
